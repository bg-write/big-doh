...
function Algorithms() {
	return (
		<div>
			...
			<p>Recursion</p>
			<p>
				A function that calls itself to break down a large problem into smaller
				bits. Achieve a small part of the larger task, then pass the partially
				completed problem to another call of itself.
			</p>
			<p>Sorting</p>
			...
			<p>
				...
				(keep splitting legos in half until you get to 1 lego, then put them all
				back together, now sorted)
			</p>
			<ul>
				<li>
					Merge Sort // Stable: Split array in half over and over until it’s
					small and sorted, then merge the small sorted pieces together on their
					way back up.
				</li>
				<li>
					...
					usually the first or last array value, but it can be any random value.
				</li>
			</ul>
			<p>
			...
			<ul>
				...
				<li>
					{' '}
					Radix Sort: Same as Bucket, but only for integers; group by each digit
				</li>
			</ul>
			<p>Searching</p>
			<ul>
